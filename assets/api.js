(function () {
  ...
  async function getRestaurant(id) {
    ...
    const res = await fetch(`/api/restaurant?${params.toString()}`);
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Kunde inte hämta restaurangen.');
    if (data.ok === false) throw new Error(data.error || 'Kunde inte hämta restaurangen.');
  ...
  window.LuunchAPI = { getNearby, getRestaurant, submitClaim };
})();
